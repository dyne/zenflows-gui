        ...
        <BrTable headArray={resourcesHead}>
            {(resources?.length !== 0) && <>{resources?.map((e) =>
                <tr key={e.node.id}>
                    ...
                    <td>{e.node.onhandQuantity?.hasNumericalValue || e.node.accountingQuantity?.hasNumericalValue} {mapUnit(e.onhandQuantity?.hasUnit.label || e.accountingQuantity?.hasUnit.label)}</td>
